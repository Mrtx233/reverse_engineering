...
my_env.plugins.def_obj_prop(Element.prototype, "getAttribute", true, true, true,
    function getAttribute(key) {
        if (this instanceof Element) {
            ...
            my_env.qiao_log(this.tagName, ".getAttribute(",key,")", "====>", val);
            return val;
        } else {
            my_env.plugins.throw_error(TypeError, "Illegal invocation");
        }
...
my_env.plugins.def_obj_prop(Element.prototype, "getElementsByTagName", true, true, true,
    function getElementsByTagName(param0) {
        if (this instanceof Element) {
            my_env.qiao_log("从",this.tagName," getElementsByTagName ", param0);

            // 用cherrio检索从当前标签去找param0 是否存在该标签
            let obj_elem = my_env.memory.nodes.get(this);
            let results = my_env.out.$(obj_elem).find(param0); // 找该标签

            let obj_col = Object.create(HTMLCollection.prototype);

            for(let i = 0 ; i < results.length; i++){
                let result_elem = results[i]; // cherrio的element对象

                // 需要包装成我们需要的原型链的对象
                let col_item_obj = null; // collection里面的对象

                let flag = true;

                // 如果已经存在了的标签. 没必要重新记载一个新对象
                ...
                if (flag) { // 该标签如果是新的. 才会走下面
                    switch (result_elem.name) {
                        case "div":
                            col_item_obj = Object.create(HTMLDivElement.prototype);
                            // 记录 对象和数据
                            my_env.memory.nodes.set(col_item_obj, result_elem);
                            break;
                        case "i":
                            col_item_obj = Object.create(HTMLElement.prototype);
                            // 记录 对象和数据
                            my_env.memory.nodes.set(col_item_obj, result_elem);
                            break;
                        ...
                        case "canvas":
                            // 为了迎合网站的API. 原型链能对的上
                            col_item_obj = Object.create(HTMLCanvasElement.prototype);
                            my_env.memory.nodes.set(col_item_obj, result_elem);

                            // 上面这个对象是不能画图的. 这个才是真正去画图的
                            let cvs = my_env.out.createCanvas();
                            my_env.memory.dom_cvs.set(col_item_obj, cvs);
                            break;
                        default:
                            my_env.qiao_log(" getElementsByTagName ", param0, "缺少", result_elem.name,);
                            break;
                    }
                }

                // 把标签对象放入obj_col里面  obj_col不需要放到nodes里面, 至少目前是这样的
                Object.defineProperty(obj_col, i, {
                    configurable: true,
                    enumerable:true,
                    writable: false,
                    value: col_item_obj
...
my_env.plugins.def_obj_prop(Element.prototype, "setAttribute", true, true, true,
    function setAttribute(key, value) {
        if (this instanceof Element) {
            if (key === 'id'){ // 防止js代码更新id
                delete my_env.memory.ids[value]; // 删掉之前的
                my_env.memory.ids[value] = this;  // 保存新的
            }
            // debugger;
            // 把数据保存在当前节点的数据中
            let c_elem = my_env.memory.nodes.get(this);
            my_env.qiao_log(this.tagName, ".setAttribute(",key, value,")");
