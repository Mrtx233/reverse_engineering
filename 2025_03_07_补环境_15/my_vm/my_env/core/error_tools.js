(function(plugins){
    ...
    plugins.handle_error = function handle_error(err){
        ...
        err.stack = err.stack.replaceAll("D:\\爬虫13期专用\\2025_03_07_补环境_15\\my_vm\\node_modules\\vm2\\lib/", "https://qikan.cqvip.com/N7xexRXkVdNQ/")
        err.stack = err.stack.replaceAll("setup-sandbox.js:393", "Y5oPTJBkyHck.cca831b.js:5");

        err.stack = err.stack.split("\n").filter(function (item){
            item = item.trim(); // 去掉空白
            return !(item.startsWith("at new Error") || item.includes('vm.js')|| item.toUpperCase().includes('node')|| item.toUpperCase().includes('vm')) ; // 不是以at new Error开头的. 我们才要
        }).join("\n");
