...
const INTERNAL_STATE_NAME = 'qiaofu_error_xxxx';
				...
				if (param.type === 'Identifier') {
					...
					if (cBody.body.length > 0) {
						insertions.push({
							...
							order: TO_LEFT,
							coder: () => `${name}=${INTERNAL_STATE_NAME}.handleException(${name});my_env.plugins.handle_error(${name});`
						});
					}
				} else {
					insertions.push({
						...
						pos: node.start,
						order: TO_RIGHT,
						coder: () => `catch(${tmpname}){${tmpname}=${INTERNAL_STATE_NAME}.handleException(${tmpname});my_env.plugins.handle_error(${tmpname});try{throw ${tmpname};}`
					});
					insertions.push({
						__proto__: null,
						pos: node.body.end,
