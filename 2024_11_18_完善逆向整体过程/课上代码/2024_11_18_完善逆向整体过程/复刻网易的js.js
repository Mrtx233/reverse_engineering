...
/**
 *
 * 我们抠出来一段代码. 但是这段代码的可用性. 不确定,
 * 需要做本地调试.  需要一个可以解释执行js的引擎...
 *
 * 最好的本地执行js的引擎: V8 -> 太素了.. -> 超级单纯的一个执行js代码的解释器
 *
 * nodejs  -> 完美的封装了V8 -> 额外的还提供了一些其他的组件. 方便我们开发
 * 我希望我的js文件也可以像python文件一样. 可以直接右键运行..而不是非要借助于浏览器运行
 * 下载地址: https://nodejs.org/en
 * 如果希望pycharm可以运行js代码
 * 需要安装node插件
 * file -> settings -> plugins -> 上方找marketplace -> 输入node -> install -> ok
 *
 * 装完了之后, js文件也可以像py文件一样 右键 -> run
 *
 * 在nodejs中提供了npm工具. 可以像python中的pip一样. 可以安装第三方库.
 * npm install xxx库
 * 默认是临时安装(一个项目一个环境)
 * 装完之后, 在文件夹下会有node_modules
 *
 * npmjs.org 官方网站
 *
 * 能不能把浏览器中所有的js撸出来. 直接用.
 * 讲道理可以.
 *
 * node环境和浏览器环境是不一样的.
 * 综上, 不可以把浏览器上的js直接抠出来用的.
 *
 * 除非.
 *  1. 你把nodejs中的特征去掉
 *  2. 把nodejs中缺失的浏览器的内容补充完整.
 * 不容易....
 * 该逻辑被称为补环境
 */
// node中导入库的语法:
// require("crypto-js")
var CryptoJS = require("crypto-js");  // 相当于import
...
function fn(ming){
    ...
    return d(ming, g1, g2, g3 );
}

// // 写完的js中, 不要预留任何测试代码...切记~~~~
// var ret = fn('123456')
// console.log(ret);
