(function (plugins) {
    function obj_proxy(obj, name) {
        if(obj.__proto__ != null){
            if(obj.__proto__.constructor.name === 'Uint8ClampedArray' ||
                ...
                obj.__proto__.constructor.name === 'Map'){
                return obj;
            }
        }
        ...
        return new Proxy(obj, {
            ...
            has(target, p) {
                my_env.qiao_log(`in检测:${"'"+p+"'"} in ${name}=>>${p in target}`)
                return Reflect.has(target, p);
            },
            deleteProperty(target, p) {
                my_env.qiao_log(`删除了${name}中的${p}`)
                return Reflect.deleteProperty(target, p);
            },
            ownKeys(target) {
                my_env.qiao_log(`正在获取${name}对象的自有属性键`);
                return Reflect.ownKeys(target);
            },
            getPrototypeOf(target) {
                my_env.qiao_log(`利用getPrototypeOf获取${name}对象中的原型链属性`);
    ...
    function func_proxy(func, name) {
        if (func.name === "eval" || func.name === "slice" || name ==='get'){
            return func;
        }
        ...
        return new Proxy(func, {
            ...
            construct(target, args, newTarget) {
                my_env.qiao_log(`${name}方法构造参数接收了`,args);
