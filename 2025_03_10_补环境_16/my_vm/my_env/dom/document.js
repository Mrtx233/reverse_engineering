...
my_env.plugins.def_obj_prop(Document.prototype, "documentElement", true, true, undefined, undefined,
    function documentElement() {
        if (this instanceof Document) {
            let obj_elem = my_env.out.$("html")[0];
            ...
            my_env.memory.nodes.set(obj, obj_elem);
...
my_env.plugins.def_obj_prop(Document.prototype, "cookie", true, true, undefined, undefined,
    ...
    function cookie(value) {
        if (this instanceof Document) {
            // 设置cookie的时候要执行的方法  'enable_6HZbKHDjIEcg=true; path'
            value = value.split("; ")[0];
...
my_env.plugins.def_obj_prop(Document.prototype, "all", true, true, undefined, undefined,
    function all() {
        if (this instanceof Document) {
            my_env.qiao_log("有人调用了. document.all");
            if (my_env.memory.document_all_count === 0){
                my_env.memory.document_all_count++;
                return undefined;
            } else {
                // 把页面上的所有的标签. 怼到all对象中
                let all_obj = Object.create(HTMLAllCollection.prototype);
                all_obj[0] = document.documentElement;
                all_obj[1] = document.head;
                all_obj[2] = document.body;
                return all_obj;
            }
        } else {
            my_env.plugins.throw_error(TypeError, "Illegal invocation");
...
my_env.plugins.def_obj_prop(Document.prototype, "createElement", true, true, true,
    function createElement(tag_name) {
        if (this instanceof Document) {
            my_env.qiao_log("createElement", tag_name);
            let c_elem;
            let obj;
            switch(tag_name){
                case "div":
                    ...
                    return obj;
                case "a":
                    c_elem = my_env.out.$("<a></a>")[0];
                    obj = Object.create(HTMLAnchorElement.prototype);
                    my_env.memory.nodes.set(obj, c_elem); // 存的时候是目标对象
                    return obj;
                case "img":
                    c_elem = my_env.out.$("<img />")[0];
                    obj = Object.create(HTMLImageElement.prototype);
                    my_env.memory.nodes.set(obj, c_elem); // 存的时候是目标对象

                    // 准备图像加载等操作
                    let img = new my_env.out.Image();
                    my_env.memory.imgs.set(obj, img);
                ...
                case "span":
                    ...
                    return obj;
                case "input":
                    c_elem = my_env.out.$("<input type='text'/>")[0];
                    obj = Object.create(HTMLInputElement.prototype);
                    my_env.memory.nodes.set(obj, c_elem);
                    return obj;
                case "form":
                    c_elem = my_env.out.$("<form></form>")[0];
                    let form_obj = Object.create(HTMLFormElement.prototype);
                    my_env.memory.nodes.set(form_obj, c_elem);
                    obj = new Proxy(form_obj, {
                        get(target, p, receiver) {
                            ...
                            my_env.qiao_log("form.", p);
                            // 从该标签内部去找
                            let objs = my_env.out.$(c_elem).find("*[name='"+p+"']");
                            if(objs.length === 0 ){
                                return Reflect.get(target, p, receiver); //  从obj取
                            }
                            // 正常应该可能会NodeList或标签对象
                            let curr_elem = objs[0];

                            for (let [k, v] of my_env.memory.nodes.entries()){
                                if(v === curr_elem){
                                    return my_env.plugins.obj_proxy(k, "form对象");
                                }
                            }
                            switch (curr_elem.tagName){
                                // 里面自行完善
            ...
            my_env.qiao_log("createElement, 缺少"+tag_name+"标签,需要去补充");
            debugger;
            return undefined;
        } else {
            my_env.plugins.throw_error(TypeError, "Illegal invocation");
...
my_env.plugins.def_obj_prop(Document.prototype, "createExpression", true, true, true,
    function createExpression(param0) {
        if (this instanceof Document) {
            let obj = Object.create(XPathExpression.prototype);
            my_env.qiao_log("有人调用了document.createExpression('",param0,"')");
            return obj;
        } else {
            my_env.plugins.throw_error(TypeError, "Illegal invocation");
        }
...
my_env.plugins.def_obj_prop(Document.prototype, "getElementById", true, true, true,
    function getElementById(id) {
        if (this instanceof Document) {
            // 通过id检索页面上的内容.
            // 拿到的是cheerio的对象
            my_env.qiao_log("document.getElementById(",id,")");
            ...
            switch(c_elem.name){
                case "div":
                    ...
                    return obj;
                case "meta":
                    obj = Object.create(HTMLMetaElement.prototype);
                    // 记录 对象和数据
                    my_env.memory.nodes.set(obj, c_elem);
                    return obj;
                case "canvas":
...
my_env.plugins.def_obj_prop(Document.prototype, "getElementsByTagName", true, true, true,
    function getElementsByTagName(tag_name) {
        if (this instanceof Document) {
            my_env.qiao_log("document.getElementsByTagName(",tag_name,")")
            // $(标签)
                ...
                if(obj === null){
                    switch (el_obj.name){
                        ...
                        case "script":
                            ...
                            break;
                        default:
                            my_env.qiao_log(" document.getElementsByTagName ", tag_name, "缺少", result_elem.name,);
                            break;
                    }
                }
