				...
				if (param.type === 'Identifier') {
					...
					if (cBody.body.length > 0) {
						insertions.push({
							...
							order: TO_LEFT,
							coder: () => `${name}=${INTERNAL_STATE_NAME}.handleException(${name});my_env.plugins.error_handle(e);`
						});
					}
				} else {
					insertions.push({
						...
						pos: node.start,
						order: TO_RIGHT,
						coder: () => `catch(${tmpname}){${tmpname}=${INTERNAL_STATE_NAME}.handleException(${tmpname});my_env.plugins.error_handle(e);try{throw ${tmpname};}`
					});
					insertions.push({
						__proto__: null,
						pos: node.body.end,
